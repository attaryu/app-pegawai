...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
    {{-- Total Gaji Diterima --}}
    <x-summary-card highlight="Rp {{ number_format($totalSalaryEarned, 0, ',', '.') }}"
        label="Total Earnings"
        small="All time"
        icon="fa-wallet" />

    {{-- Gaji Bulan Ini --}}
    <x-summary-card highlight="Rp {{ number_format($monthlySalary, 0, ',', '.') }}"
        label="This Month Salary"
        small="{{ now()->format('F Y') }}"
        icon="fa-money-bill-wave" />

    {{-- Total Kehadiran --}}
    <x-summary-card highlight="{{ $totalAttendance }}"
        label="Total Attendance"
        small="All time"
        icon="fa-clipboard-check" />

    {{-- Kehadiran Bulan Ini --}}
    <x-summary-card highlight="{{ $monthlyAttendance }}"
