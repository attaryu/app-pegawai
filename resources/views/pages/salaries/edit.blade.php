...
@section('content')
    <x-card>
        ...
        <form action="{{ route('dashboard.admin.salaries.update', $salary->id) }}" method="POST" class="mt-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            <fieldset class="space-y-4">
                <div class="flex gap-4 w-full">
                    ...
                    <x-select id="bulan" name="bulan" error="{{ $errors->first('bulan') }}"
                        ...
                        ]" value="{{ old('bulan', $salary->bulan) }}" class="w-full" />
                </div>

                <div class="flex gap-4 w-full">
                    <x-input id="gaji_pokok" name="gaji_pokok" error="{{ $errors->first('gaji_pokok') }}"
                        label="Gaji Pokok" placeholder="0" type="number"
                        value="{{ old('gaji_pokok', $salary->gaji_pokok) }}" class="w-full" />

                    <x-input id="gaji_tunjangan" name="gaji_tunjangan" error="{{ $errors->first('gaji_tunjangan') }}"
                        label="Gaji Tunjangan" placeholder="0" type="number"
                        value="{{ old('gaji_tunjangan', $salary->gaji_tunjangan) }}" class="w-full" />

                    <x-input id="potongan" name="potongan" error="{{ $errors->first('potongan') }}"
                        label="Potongan" placeholder="0" type="number"
                        value="{{ old('potongan', $salary->potongan) }}" class="w-full" />
                </div>
            </fieldset>
